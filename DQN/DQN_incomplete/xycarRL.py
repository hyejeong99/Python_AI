...
class learning_xycar:
	...
	self.world_time += self.dt
        ...
        self.done = False
        if int(self.suc_code) > 0 or (self.world_time > 5):#plus
            self.done = True
    ...
    def set_xycar_steering(self, s):
	self.xycar.set_steering(s)	
	#self.xycar_steering += s

    def set_xycar_back(self):
	max_time = time.time() + (2.0)
	self.xycar.set_max_velocity(500)
	self.xycar.set_steering_angle(70)
	#while (self.calibration_time()*(time.time()-self.start_time)) < 2.0
	while time.time() < max_time:
	    setT = self.calibration_time()*(time.time()-self.start_time)
	    print("TIME:",setT)
	    self.set_xycar_velocity(-100)
	    self.set_xycar_steering(-100+self.xycar_steering)
	    print("SPEED:", self.xycar.get_velocity())
	    print("ANGLE:", self.xycar_steering)

    def get_xycar_velocity(self):
	return self.xycar.get_velocity()

    def set_xycar_velocity(self, v):
	self.xycar.set_velocity(v)

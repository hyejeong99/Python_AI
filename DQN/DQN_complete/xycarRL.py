...
class learning_xycar:
    ...
    def set_map(self, name):
        ...
        if "teleport" in self.conf:
            self.set_teloport()

    def set_teloport(self):
        teleport = self.conf['teleport']
        self.before_x = teleport['beforeX']
        self.before_y = teleport['beforeY']
        
        self.after_x1 = min(teleport['afterX1'], teleport['afterX2'])
        self.after_y1 = min(teleport['afterY1'], teleport['afterY2'])
        self.after_x2 = max(teleport['afterX1'], teleport['afterX2'])
        self.after_y2 = max(teleport['afterY1'], teleport['afterY2'])

        self.before_pose = teleport['beforeP']
    ...
    def step(self, action):
        ...
        #print(self.world_time, self.suc_code, self.step_count)

        self.done = False
        if (int(self.suc_code) > 0) or (self.world_time > 40):
            self.done = True
        ...
        x = self.xycar_position[0]
        y = self.xycar_position[1]
        yaw = self.xycar_yaw

        if ("teleport" in self.conf) and ((self.after_x1 < x < self.after_x2) and (self.after_y1 < y < self.after_y2)):
            self.xycar.set_position(x, y, yaw)
            self.round_cnt += 1

        state += [self.get_round_count()]
    ...
    def get_round_count(self):
        return self.round_cnt

    def get_step_number(self):
        return self.step_count

    def get_xycar_velocity(self):
        return self.xycar.linear_velocity

...
def parse_args():
    ...
    parser.add_argument("--video_path", '-i', type=str, default="Ta3.mp4", help="Path to demo video")
    parser.add_argument("--weight_path", '-w', type=str, default="experiments/exp1/exp1.pth", help="Path to model weights")
...
def visualize(img, seg_pred, exist_pred):
    ...
    img = cv2.add(img, (-20, -20, -20, 0))
    lane_img = np.zeros_like(img)
    color = np.array([[255, 125, 0], [0, 255, 0], [0, 0, 255], [0, 255, 255]], dtype='uint8')
    coord_mask = np.argmax(seg_pred, axis=0)
    exist = [1 if exist_pred[0, i] > 0.5 else 0 for i in range(0, 4)]
    lines = []
